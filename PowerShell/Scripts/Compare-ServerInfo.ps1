#DevOps_Compare-ServerInfo – script contents:
# Author: Cindy Muesing
# Date: 12/31/19

###############################################################################################

function DevOps_Compare-ServerInfo {
    [cmdletbinding()]
                #set variables
    ...
    $ServerDiff = Invoke-Command  {
        $services = Compare-Object -ReferenceObject (Get-Content H:\PS\outputs\$($computernameSource)_Services_$CompareDate.csv) -DifferenceObject (Get-Content H:\PS\outputs\$($computernameTarget)_Services_$CompareDate.csv)
        $wmiapps = Compare-Object -ReferenceObject (Get-Content H:\PS\outputs\$($computernameSource)_wmiapps_$CompareDate.csv) -DifferenceObject (Get-Content H:\PS\outputs\$($computernameTarget)_wmiapps_$CompareDate.csv)
        $features = Compare-Object -ReferenceObject (Get-Content H:\PS\outputs\$($computernameSource)_Features_$CompareDate.txt) -DifferenceObject (Get-Content H:\PS\outputs\$($computernameTarget)_Features_$CompareDate.txt)
        $programs = Compare-Object -ReferenceObject (Get-Content H:\PS\outputs\$($computernameSource)_Programs_$CompareDate.txt) -DifferenceObject (Get-Content H:\PS\outputs\$($computernameTarget)_Programs_$CompareDate.txt)
        $websites = Compare-Object -ReferenceObject (Get-Content H:\PS\outputs\$($computernameSource)_WebsiteInfo_$CompareDate.txt) -DifferenceObject (Get-Content H:\PS\outputs\$($computernameTarget)_WebsiteInfo_$CompareDate.txt)
        $appPools = Compare-Object -ReferenceObject (Get-Content H:\PS\outputs\$($computernameSource)_AppPools_$CompareDate.txt) -DifferenceObject (Get-Content H:\PS\outputs\$($computernameTarget)_AppPools_$CompareDate.txt)
    return $services,$wmiapps,$features,$programs, $websites, $appPools
    }

    $ServerDiff[0] | Out-File H:\PS\outputs\$($computernameSource)_ServicesDiff_$CompareDate.txt -Force
    $ServerDiff[1] | Out-File H:\PS\outputs\$($computernameSource)_wmiappsDiff_$CompareDate.txt -Force
    $ServerDiff[2] | Out-File H:\PS\outputs\$($computernameSource)_FeaturesDiff_$CompareDate.txt  -Force
    $ServerDiff[3] | Out-File H:\PS\outputs\$($computernameSource)_ProgramsDiff_$CompareDate.txt -Force
    $ServerDiff[4] | Out-File H:\PS\outputs\$($computernameSource)_WebsitesDiff_$CompareDate.txt -Force
    $ServerDiff[5] | Out-File H:\PS\outputs\$($computernameSource)_AppPoolsDiff_$CompareDate.txt -Force
}
###############################################################################################

DevOps_Compare-ServerInfo

# SIG # Begin signature block
# MIIRcgYJKoZIhvcNAQcCoIIRYzCCEV8CAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUBWXX7+EQjXHd7wfKaah95mXM
# 9v+ggg7MMIIGfDCCBGSgAwIBAgIKERQ+ugAAAAAABzANBgkqhkiG9w0BAQsFADAd
...
# XMmMmpQLkMdL0sgXeB/BHiVR4TANBgkqhkiG9w0BAQEFAASCAQB+l3xgphym02uf
# fAJ63+R+Jl/JweXvoY3UUD3ZtB/fiO3KSLpzWe0EWtCSY4aCYGSz3LDluYy/5Mv+
# XO7mnINqvaehL4fmETHWSJsoxKwnY5RSEru4c99cR+CCRsPToEqgU+nigpK00Ird
# AszYGoYfTMqVFyw9fjYnYUpXYHcB5YISLPVwGWfyccOrFLbOoxcbw9K+dkC0zabG
# HzAiy1jCN+TSpoYW+0IUI5SPn3vpPZcPEqQWHZxTkNZUf1nSUpMDYoPSplN5faij
# Dr3shaSQjMl356W16zgIxS4WGLsyOIUOydt3/mgNFO9eZ/nK2PAyWNtrEwevw0Zn
# 6FDBifxW
