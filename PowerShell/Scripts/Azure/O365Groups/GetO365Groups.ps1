...
<# exporst list of users in azure group to csv file#>
Param 
( 
...
Function main() 
{
 ...
 #Set export path
 $ExportPath = Read-Host "Export file path"

 #Set output file 
 $ExportCSV="$ExportPath $((Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv" #Detailed report
 $ExportSummaryCSV="$ExportPath_$((Get-Date -format yyyy-MMM-dd-ddd` hh-mm` tt).ToString()).csv" #Summary report

  #Get a list of RecipientTypeDetail
  $PresetFile = "c:\Users\mjhus\OneDrive\PowerShell\Azure\O365Groups\Presets"
  $RecipientTypeArray=Get-Content -Path $PresetFile.txt -ErrorAction Stop
  $Result=""  
  $Results=@()
  $Count=0
